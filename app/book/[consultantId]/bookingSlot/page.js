...
import { faCalendarAlt, faClock, faSpinner, faCheckCircle, faTimesCircle, faDollarSign,faIndianRupeeSign } from '@fortawesome/free-solid-svg-icons';
...
const Page = () => {
  ...
  // Calculate the booking amount based on the selected slot
  const bookingAmount = useMemo(() => {
    return selectedSlotObject?.rate ?? 0;
  }, [selectedSlotObject]);


  // 3. Normalize Previous Bookings Data (using dummy data, ensure to replace with real API data)
  const previousBookings = useMemo(() => {
    // Replace with data?.previousBookings ?? []; when available
    return [
        { _id: 'bk123456', date: new Date(Date.now() - 86400000 * 5).toISOString(), slot: {startHour: 10, endHour: 11}, status: 'completed' },
        { _id: 'bk123457', date: new Date(Date.now() - 86400000 * 10).toISOString(), slot: {startHour: 14, endHour: 15}, status: 'cancelled' },
    ].map(b => ({
        ...b,
        // Adapt display for previous bookings to match the schema slot object
        displaySlot: formatSlotForDisplay(b.slot)
    }));
  }, [data]);
  ...
  return (
    <div className="bg-gray-900">
      <div className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8 min-h-screen">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl overflow-hidden mb-8 border border-richblack-600">
          <div className="flex flex-col lg:flex-row ">
            
            {/* Left Column: Consultant Profile Summary */}
            <div className="lg:w-1/3 p-6 sm:p-8 bg-blue-50 dark:bg-slate-800 border-b lg:border-r border-neutral-100 dark:border-slate-700">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-lg text-blue-600 dark:text-blue-400 font-medium">Consultant</p>
                <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-2 italic">{data.bio ?? consultant.bio}</p>
                
                {/* Categories/Specializations */}
                <Rating
                  className="mt-3"
                  initialRating={ratingCount.average || 0}
                  readonly={true}
                ...
                <div className="mt-4 flex gap-2 flex-wrap justify-center">
                  {(data.categories ?? consultant.categories ?? []).map((c) => (
                    <span 
                      key={c._id ?? c.name} 
                      className="px-3 py-1 rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300 text-xs font-semibold"
                    >
                      {c.name ?? c}
                    </span>
                  ))}
            ...
            <div className="lg:w-2/3 p-6 sm:p-8">
              ...
              {/* Step 1: Date Picker */}
              <div className="mb-6">
                <h4 className="flex items-center text-lg font-semibold mb-3 text-neutral-700 dark:text-neutral-300">
                  <FontAwesomeIcon icon={faCalendarAlt} className="mr-2 text-blue-500 w-4 h-4" /> Step 1: Select Date
                </h4>
                <label htmlFor="booking-date" className="block text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-2">
                ...
                <input
                  ...
                  min={today}
                  max={maxDate}
                  className="w-full cursor-pointer text-gray-600 dark:text-white sm:w-1/2 p-3 border border-neutral-300 dark:border-slate-700 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-slate-800 transition-shadow shadow-sm"
                />
              </div>
                    ...
                    slotsForDisplay.map((slotObj) => (
                      <button
                        ...
                        className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-all duration-150 relative ${
                          slotObj.isBooked
                            ? "bg-neutral-100 dark:bg-slate-700 border-neutral-300 dark:border-slate-600 text-neutral-500 cursor-not-allowed opacity-70" // Booked/Disabled style
                            : selectedSlotObject?._id === slotObj._id // Compare by object ID
                              ? "bg-blue-600 text-white border-blue-600 shadow-lg" // Selected style
                              : "bg-white dark:bg-slate-800 border-neutral-200 dark:border-slate-700 text-gray-800 dark:text-white hover:bg-neutral-50 dark:hover:bg-slate-700" // Available style
              ...
              <div className="mb-6">
                ...
                <textarea
                    ...
                    className="w-full p-3 border border-neutral-300 dark:border-slate-700 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-slate-800 text-gray-800 dark:text-white"
                />
              </div>

              {/* Step 4: Summary & Action Button */}
              <div className="mt-8 pt-4 border-t border-neutral-100 dark:border-slate-700">
                <div className="flex justify-between items-center text-lg font-bold mb-4 text-gray-800 dark:text-white">
                    <span>Total Booking Amount:</span>
                    <span className="text-green-600 dark:text-green-400"><FontAwesomeIcon icon={faIndianRupeeSign} />{bookingAmount}</span>
                </div>
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className={`cursor-pointer flex-grow px-6 py-3 rounded-lg text-white font-bold text-lg transition-colors ${
                      !selectedSlotObject || booking || !selectedDate || !description.trim() || bookingAmount <= 0
                        ...
                        : "bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 shadow-md shadow-blue-200/50"
                    }`}
                  >
                    {booking 
                  ...
                  <button
                    onClick={() => router.back()}
                    className="px-4 py-3 rounded-lg transition duration-300 border border-neutral-300 bg-red-500 text-sm text-white font-semibold dark:text-neutral-200 hover:bg-red-600"
                  >
                    Cancel
                  </button>
                </div>
          ...
          {previousBookings.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {previousBookings.map((booking) => (
                <div 
                  key={booking._id} 
                  className="p-4 bg-white dark:bg-slate-900 border border-neutral-100 dark:border-slate-700 rounded-lg shadow-sm hover:shadow-md transition-shadow"
                >
            ...
            <div className="p-6 text-center border-2 border-dashed border-neutral-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900">
